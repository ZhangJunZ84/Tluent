...
function IconModule.Icon(Icon, Type, DefaultFormat)
	...
	-- Support direct/raw asset paths (like rbxassetid://) immediately
	if type(Icon) == "string" and (string.find(Icon, "rbxassetid://") or string.find(Icon, "rbxasset://") or string.find(Icon, "http://") or string.find(Icon, "https://")) then
	...
	elseif iconSet and type(iconSet[targetName]) == "string" and (string.find(iconSet[targetName], "rbxassetid://") or string.find(iconSet[targetName], "rbxasset://") or string.find(iconSet[targetName], "http://") or string.find(iconSet[targetName], "https://")) then
		return DefaultFormat
				and {
					iconSet[targetName],
					{ ImageRectSize = Vector2.new(0, 0), ImageRectPosition = Vector2.new(0, 0) },
...
function IconModule.Image(IconConfig)
	...
	local IconLabel = Icon.Icon and IconModule.Icon2(Icon.Icon, Icon.Type) or nil
	local isrbxassetid = typeof(IconLabel) == "string" and string.find(IconLabel, "rbxassetid://")

	local ImageVal = ""
	local ImageRectSizeVal = nil
	local ImageRectOffsetVal = nil

	if IconLabel then
		if isrbxassetid then
			ImageVal = IconLabel
		else
			ImageVal = IconLabel[1] or ""
			if IconLabel[2] then
				ImageRectSizeVal = IconLabel[2].ImageRectSize
				ImageRectOffsetVal = IconLabel[2].ImageRectPosition
			end
		end
	end
	...
	if IconModule.New then
		...
		local IconFrame = New("ImageLabel", {
			...
			Image = ImageVal,
			ImageRectSize = ImageRectSizeVal,
			ImageRectOffset = ImageRectOffsetVal,
		})

		if not isrbxassetid and IconLabel and IconLabel[2] and IconLabel[2].Parts then
			for _, part in next, IconLabel[2].Parts do
				...
				if IconPartLabel then
					local IconPart = New("ImageLabel", {
						...
						ImageRectSize = IconPartLabel[2] and IconPartLabel[2].ImageRectSize or nil,
						ImageRectOffset = IconPartLabel[2] and IconPartLabel[2].ImageRectPosition or nil,
						Parent = IconFrame,
					})
				end
			end
		...
		IconFrame.Image = ImageVal
		if ImageRectSizeVal then IconFrame.ImageRectSize = ImageRectSizeVal end
		if ImageRectOffsetVal then IconFrame.ImageRectOffset = ImageRectOffsetVal end
